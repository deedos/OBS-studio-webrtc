...
# libopus
curl -L -O http://downloads.xiph.org/releases/opus/opus-1.2.1.tar.gz
...
# libvorbis
curl -L -O http://downloads.xiph.org/releases/vorbis/libvorbis-1.3.6.tar.gz
...
mkdir -p build
cd ./build
../configure --disable-shared --prefix="/tmp/obsdeps" --libdir="/tmp/obsdeps/bin"
make -j 12
make install
